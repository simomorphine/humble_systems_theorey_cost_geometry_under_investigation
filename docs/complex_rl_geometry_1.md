...
These two burdens are *geometrically orthogonal*. No weighted sum $c + \mu d$ traces the full Pareto frontier between them for all environments. The right response to orthogonality is not to fight it with a tuning knob — it is to embrace it mathematically. Encode them as the real and imaginary parts of a complex number:
 
$$z = c + id \in \mathbb{C}$$
 
The modulus $|z| = \sqrt{c^2 + d^2}$ is the Euclidean distance from the origin in the cost-debt plane. Minimising $|z|$ is principled, parameter-free, and geometrically natural. No weighting parameter. No tuning. The geometry does the work.
...
| **Proven** | Under the HST Equilibrium Axiom, $\vert Q \vert^2$ is a Lyapunov function and the fixed point lies in the upper half-plane |
...
**Lemma 1.2** *(Modulus of expectation).* For any $\mathbb{C}$-valued random variable $Z$ with $\mathbb{E}[|Z|] < \infity$:
...
> **Two distinct criteria.** The modulus-of-expectation $J^\pi(s) = |\mathbb{E}[G^\pi_t \mid S_t = s]|$ yields a tractable linear Bellman equation. The expected-modulus $\tilde{J}^\pi(s) = \mathbb{E}[|G^\pi_t| \mid S_t = s]$ requires a distributional treatment and is an open problem. By Lemma 1.2, $J^\pi \leq \tilde{J}^\pi$.
...
The decision rule is correct in spirit — but $\mu$ is a free parameter that traces a different point on the Pareto frontier for each value.
...
The information debt of a transition is determined entirely by the difference in epistemic potential between origin and destination. A state with high $\phi$ is one where the agent is still uncertain. A transition that *increases* uncertainty yields negative $d$, which increases the imaginary part of the utility and penalises the action through the modulus.
 
> **Important caveat.** The potential difference form is a structural condition enabling clean algebra — not a restriction derived from BAMDP or Bayesian reasoning. The telescoping identity of Section 7 holds for *any* $\phi : \mathcal{S} \to \mathbb{R}$, regardless of interpretation. The semantic interpretation as entropy is introduced separately in Section 8.
 
**Assumption 6.2** *(Learning assumption).* The epistemic potential is non-increasing in expectation along any policy trajectory:
...
$$Q^\pi_I(s,a) = \mathbb{E}^\pi\!\left[\sum_{k=0}^\infty \lambda^k \bigl(\phi(S_{t+k}) - \phi(S_{t+k+1})\bigr)\right]$$
 
**Step 2.** Split the sum:
 
$$= \mathbb{E}^\pi\!\left[\sum_{k=0}^\infty \lambda^k \phi(S_{t+k}) - \sum_{k=0}^\infty \lambda^k \phi(S_{t+k+1})\right]$$
...
$$\lambda^k - \lambda^{k-1} = \lambda^{k-1}(\lambda - 1) = -(1-\lambda)\lambda^{k-1}$$
 
**Step 6.** Shift index $j = k-1$ to obtain the stated result. $\square$
 
The algebraic engine is Step 3: reindexing creates the factor $(\lambda^k - \lambda^{k-1})$. The initial potential $\phi(s)$ survives because $S_t = s$ is fixed — it has no cancellation partner in the reindexed sum.
 
**Corollary 7.2** *(Non-negativity).* Under Assumption 6.2, $Q^\pi_I(s,a) \geq 0$ for all $(s, a, \pi)$.
 
*Proof.* Assumption 6.2 makes $\phi$ a supermartingale along any policy trajectory, so the discounted average on the right is at most $\phi(s)$. $\square$
...
**Corollary 9.3** *(The contraction gap vanishes at equilibrium).* The contraction gap of Section 4 arises when two complex values selected by $\arg\min |\cdot|$ point in nearly opposite directions. Proposition 9.2 places $Q^*$ in the upper half-plane. As $Q^*_I \to 0$, all values approach the positive real axis and the maximum phase difference approaches zero. The contraction gap **vanishes precisely at the fixed point that Axiom 9.1 guarantees the system is converging toward**.
...
- $\mathcal{L}(s,a)$ measures **total distance from full equilibrium** — zero cost and zero epistemic uncertainty simultaneously
 
Under Axiom 9.1, $\mathcal{L} \to 0$ only when both cost is minimised and the system is epistemically certain.
...
**Candidate resolutions.** A *cone restriction* would bound the maximum phase difference: if $Q^*(s,a) \in \{z \in \mathbb{C} : |\mathrm{Arg}(z)| \leq \theta\}$ for some $\theta < \pi/4$, the cross-action term can be controlled (Corollary 9.3 shows $\theta \to 0$ at equilibrium). A *phase-weighted norm* $\|Q\|_{\infty,\varphi} = \max_{s,a} e^{\beta |\mathrm{Arg}(Q(s,a))|} |Q(s,a)|$ for $\beta > 0$ may allow a direct contraction proof. *Schauder's theorem* gives existence without contraction since $T$ maps a bounded ball into itself; uniqueness then requires a separate argument. A *smoothed operator* with $\pi_\tau(a \mid s) \propto \exp(-|Q(s,a)|/\tau)$ is Lipschitz and its limit as $\tau \to 0$ recovers the greedy selector.
...
The **imaginary Q-value**, under the potential difference form $d(s,a,s') = \phi(s) - \phi(s')$, telescopes to a closed-form expression. This is *pure algebra*: it holds for any $\phi$, regardless of interpretation.
